...
class KeypointEncoder(nn.Module):
    def __init__(self, pretrained, with_drop):
        ...
        if pretrained:
            ...
            pre = torch.load(cur_path + '/resnet18-5c106cde.pth')
            self.rn.load_state_dict(pre)
...
class KeypointDecoder(nn.Module):
    def __init__(self):
        ...
        # Layer4
        self.upconv4_0 = conv_bn_elu(self.num_ch_enc[4], self.num_ch_dec[4])  # 512 256
        self.upconv4_1 = conv_bn_elu(self.num_ch_dec[4] + self.num_ch_enc[3], self.num_ch_dec[4])  # 512 256

        # Layer3
        self.upconv3_0 = conv_bn_elu(self.num_ch_dec[4], self.num_ch_dec[3])  # 256 256
        self.upconv3_1 = conv_bn_elu(self.num_ch_dec[3] + self.num_ch_enc[2], self.num_ch_dec[3])  # 384 256
    ...
    def forward(self, input_features):
        ...
        x = torch.cat(x, 1)
        x = self.upconv1_1(x)  # 128-64 /2  # 这么精细的嘛
        # Descriptor features
