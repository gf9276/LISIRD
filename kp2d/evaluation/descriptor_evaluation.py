...
def compute_matching_score(data, keep_k_points=1000):
    ...
    bf = cv2.BFMatcher(cv2.NORM_L2, crossCheck=False)  # 我呸，就应该crosscheck
    ...
    matches_nbr = (0.5 * (np.sum(vis) + np.sum(vis_warped)))  # 配对点数
    correct_matches_nbr = (0.5 * (count1 + count2))

    return ms, correct_matches_nbr, matches_nbr
...
def compute_homography(data, keep_k_points=1000):
    ...
    # x = (np.expand_dims(desc.T, 2) - np.expand_dims(warped_desc.T, 1))
    ...
    matches_idx = np.array([m.queryIdx for m in matches])
    m_keypoints = keypoints[matches_idx, :]
    # ooo = np.arange(warped_keypoints.shape[0])
    # desc_more = desc[np.setdiff1d(ooo, matches_idx), :]
    # keypoints_more = keypoints[np.setdiff1d(ooo, matches_idx), :]

    matches_idx = np.array([m.trainIdx for m in matches])
    m_warped_keypoints = warped_keypoints[matches_idx, :]
    # ooo = np.arange(warped_keypoints.shape[0])
    # warped_desc_more = warped_desc[np.setdiff1d(ooo, matches_idx), :]
    # warped_keypoints_more = warped_keypoints[np.setdiff1d(ooo, matches_idx), :]

    # matches_more = bf.match(desc_more, warped_desc_more)
    # matches_idx = np.array([m.queryIdx for m in matches_more])
    # m_keypoints_more = keypoints_more[matches_idx, :]
    # matches_idx = np.array([m.trainIdx for m in matches_more])
    # m_warped_keypoints_more = warped_keypoints_more[matches_idx, :]

    # m_keypoints = np.concatenate([m_keypoints, m_keypoints_more], 0)
    # m_warped_keypoints = np.concatenate([m_warped_keypoints, m_warped_keypoints_more], 0)
...
def bf_match(desc, warped_desc):
    dist = torch.mm(desc, warped_desc.t())
    dist = torch.sqrt(2 - 2 * torch.clamp(dist, min=-1, max=1))

    range_idx = torch.arange(0, desc.shape[0], device=desc.device)

    src_min_idx = dist.argmin(1)  # 原点对应距离最小的目标点下角标
    tgt_min_idx = dist.argmin(0)
    mask_src = (tgt_min_idx[src_min_idx] == range_idx)

    tgt_good_idx = src_min_idx[mask_src]
    src_good_idx = range_idx[mask_src]

    dist_good = dist[mask_src, tgt_good_idx].clone()
    dist_good, dist_good_sort_idx = dist_good.sort()  # 升序

    src_good_idx = src_good_idx[dist_good_sort_idx]
    tgt_good_idx = tgt_good_idx[dist_good_sort_idx]

    return src_good_idx, tgt_good_idx
