...
def compute_repeatability(data, keep_k_points=300, distance_thresh=3):
    ...
    norm = np.linalg.norm(true_warped_keypoints - warped_keypoints, ord=None, axis=2)  # 求欧式距离
    count1 = 0
    count2 = 0
    le1 = 0
    le2 = 0
    if N2 != 0:
        ...
        correct1 = (min1 <= distance_thresh)  # 对true_warped_keypoints来讲，可能对应点，一个对一个
        count1 = np.sum(correct1)  # 总数量
        le1 = min1[correct1].sum()  # 这些合适点的距离和，对应点的距离
    if N1 != 0:
        min2 = np.min(norm, axis=0)
        correct2 = (min2 <= distance_thresh)  # 对warped_keypoints再来一次操作
        count2 = np.sum(correct2)
        le2 = min2[correct2].sum()
    if N1 + N2 > 0:  # 他会降，降低，emmm，既然N1 N2不会变，那就是count变少了，那就是距离里的点变少了，可能是不再集中分布了？我也觉得均匀点好
        repeatability = (count1 + count2) / (N1 + N2)  # 这里固定600，前面就是对应点数量，rep小说明前面小，挺搞笑的，这不是矛盾吗
        if (count1 + count2) > 0:
            loc_err = (le1 + le2) / (count1 + count2)  # 属于是平均距离，如果count变小，留下的可能是精品，真正合适的点
        else:
            loc_err = -1
    else:
        repeatability = -1
